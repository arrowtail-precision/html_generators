    ...
    str(
        h.Document(
            ...
            h.Body(
                ...
                (h.Div(x, div_index=i) for i, x in enumerate(["a", "b", "c"])),
...
assert_equal(
    str(h.Document(h.Body("body"), class_="foo")),
    """<!DOCTYPE html>
...
assert_equal(
    str(h.Comment("My favorite operators are > and <!")),
...
assert_equal(
    str(h.Div(h.I("a"), class_="a").with_classes("b")),
    '<div class="a b"><i>a</i></div>',
)
...
assert_equal(
    h.styles(
        "display: block", False and "NOT", None and "NOT", True and "color: green"
    ),
...
assert_equal(str(h.template("<a{{a}}b{{a}}{{b}}", a="A", b="B")), "&lt;aAbAB")
